...
export const deleteProject = (projectIds) => dispatch => {
    dispatch({
        type: DELETE_PROJECTS_START
    });
    return Promise.all(projectIds.map(projectId => api.deleteProject(projectId))).then(
        response => {
            dispatch({
                type: DELETE_PROJECTS_SUCCESS,
                payload: { success: response, id: projectIds }
            });
